...
/** THIS SCRIPT IS PATCHED TO IGNORE DUPLICATE ACTORS, PICKING THE OLDEST ONE. */
...
class MigrateActors extends LoggedUpdateMaintenance
{
	...
	protected function doDBUpdates()
	{
		...
		$errors += $this->migrateToTemp(
			'revision',
			'rev_id',
			['revactor_timestamp' => 'rev_timestamp', 'revactor_page' => 'rev_page'],
			'rev_user',
			'rev_user_text',
			'revactor_rev',
			'revactor_actor'
	...
	protected function migrateLogSearch()
	{
		...
		while (true) {
			...
			if ($del) {
				$dbw->delete(
					'log_search',
					['ls_field' => 'target_author_id', 'ls_value' => $del],
					__METHOD__
				);
				$countUpdated += $dbw->affectedRows();
			}
		...
		while (true) {
			...
			if ($del) {
				$dbw->delete(
					'log_search',
					['ls_field' => 'target_author_ip', 'ls_value' => $del],
					__METHOD__
				);
				$countUpdated += $dbw->affectedRows();
			}
